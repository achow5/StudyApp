...
public class StudyDatabase extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // Add some subjects
        String[] subjects = { "Discrete Math", "Chemistry", "World History" };
    ...
    public List<Question> getRandomQuestions(String subject) {
        List<Question> questions = new ArrayList<>();

        SQLiteDatabase db = this.getReadableDatabase();
        String sql = "select * from " + QuestionTable.TABLE +
                " where " + QuestionTable.COL_SUBJECT + " = ?" + " order by RANDOM()";
        Cursor cursor = db.rawQuery(sql, new String[] { subject });
        if (cursor.moveToFirst()) {
            do {
                Question question = new Question();
